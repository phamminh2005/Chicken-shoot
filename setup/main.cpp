...
public:
	...
	static double MaxMin(double a, double b, int max) {
		if ((a > b && max) || (a < b && !max))
			return a;
		return b;
	}
...
public:
	...
	void CheckCollider(Player* player) {
		for (int i = 0; i < listEnemy.size(); i++)
			if (listEnemy[i]->isActive)
				if (Math::CheckColliderBox2D(listEnemy[i]->texture, player->texture)) {
					player->isActive = false;
...
void Play() {
	...
	Player *player = new Player(gRenderer, new Information(10, 50, 20), "./Images/Player/Ship1/Ship1.png", Vector2D(300, 800));
